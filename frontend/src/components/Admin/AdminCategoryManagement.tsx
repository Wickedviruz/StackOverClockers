...
const AdminCategoryManagement: React.FC = () => {
  ...
  if (!isAdmin) {
    return <div className="max-w-4xl mx-auto mt-10">Unauthorized</div>;
  }

  return (
    <div className="max-w-4xl mx-auto mt-10">
      <h2 className="text-3xl mb-6">Admin - Manage Categories</h2>
      <div className="mb-6">
      ...
      <div>
        <h3 className="text-2xl mb-4">Existing Categories</h3>
        <ul>
          {categories.map(category => (
            <li key={category.id} className="border-b py-2 flex justify-between items-center">
              <div>
                <h4 className="text-xl font-bold">{category.name}</h4>
                <p>{category.description}</p>
              </div>
              {/* Lägg till funktioner för redigering och borttagning om så önskas */}
            </li>
          ))}
        </ul>
      </div>
